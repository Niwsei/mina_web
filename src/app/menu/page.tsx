...
export default async function MenuPage() {
  ...
  <div className="section-shell pattern-grid overflow-hidden space-y-10 text-(--brand-cream)">
          <div className="relative">
            ...
            <div className="grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_320px]">
              <div className="space-y-6 anim-fadeUp">
                <div className="flex flex-wrap items-center gap-3 text-(--brand-cream)/70">
                  <span className="eyebrow">กาแฟเข้มข้นประจำบาร์</span>
                  <span className="badge badge-fire">อัปเดตทุกวัน</span>
                </div>
                <h1 className="section-title text-3xl md:text-4xl text-(--brand-cream)">เมนู</h1>
                ...
                <ul className="grid gap-4 text-sm text-(--brand-cream)/75 md:grid-cols-2">
                  <li className="flex items-start gap-3">
                    <span className="mt-1 inline-flex h-2.5 w-2.5 rounded-full bg-(--brand-orange)" aria-hidden="true" />
              ...
              <aside className="floating-card animate-float-slow space-y-5 overflow-hidden border-white/10 bg-[rgba(18,10,6,0.85)] p-6 text-(--brand-cream)/85">
                <span className="taste-chip">Barista Picks</span>
                <p className="text-sm leading-relaxed text-(--brand-cream)/75">
                ...
                <div className="space-y-3">
                  {signatureSuggestions.length > 0 ? (
                    signatureSuggestions.map((item) => (
                      <div
                        key={item.id}
                        className="rounded-[calc(var(--radius)-4px)] border border-white/10 bg-white/5 px-4 py-3 text-sm text-(--brand-cream)/85"
                      >
                    ...
                    <div className="rounded-[calc(var(--radius)-4px)] border border-white/10 bg-white/5 px-4 py-3 text-sm text-(--brand-cream)/85">
                      <p className="font-semibold text-(--brand-cream)">พร้อมเสิร์ฟ</p>
